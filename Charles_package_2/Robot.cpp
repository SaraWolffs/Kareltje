...
void ROBOT :: maakBallenPad ()			// toegevoegd door Peter
{
	...
	for (int ix = 1; ix < WereldBreedte-2; ix++)					Wereld[ix][WereldHoogte/2]  = Bal;
	for (int iy = WereldHoogte/2; iy < WereldHoogte-3; iy++)		Wereld[WereldBreedte-3][iy] = Bal;
	for (int ix = WereldBreedte-3; ix >= WereldBreedte / 2; ix--)	Wereld[ix][WereldHoogte-4]  = Bal;
	for (int iy = WereldHoogte-4; iy >= 1; iy--)					Wereld[WereldBreedte/2][iy] = Bal;
	for (int ix = WereldBreedte / 2; ix >= 2; ix--)		    		Wereld[ix][1]               = Bal;
	for (int iy = 1; iy <= WereldHoogte / 2 - 3; iy++)				Wereld[2][iy]               = Bal;
